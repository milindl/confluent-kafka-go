...
func (a *AdminClient) cToConsumerGroupDescriptions(cGroups **C.rd_kafka_ConsumerGroupDescription_t, cGroupCount C.size_t) (result []ConsumerGroupDescription) {
	result = make([]ConsumerGroupDescription, cGroupCount)
	for idx := 0; idx < int(cGroupCount); idx++ {
		cGroup := C.ConsumerGroupDescription_by_idx(cGroups, C.size_t(idx), cGroupCount)

	}
}

func (a *AdminClient) cToConsumerGroupListings(
	cGroups **C.rd_kafka_ConsumerGroupListing_t, cGroupCount C.size_t) (result []ConsumerGroupListing) {
	result = make([]ConsumerGroupListing, cGroupCount)

	for idx := 0; idx < int(cGroupCount); idx++ {
		cGroup := C.ConsumerGroupListing_by_idx(cGroups, C.size_t(idx), cGroupCount)
...
func (a *AdminClient) cToErrorList(
	...
	for idx := 0; idx < int(cErrCount); idx++ {
		cErr := C.error_by_idx(cErrs, C.size_t(idx), cErrCount)
...
func (a *AdminClient) ListConsumerGroups(
	ctx context.Context,
	options ...ListConsumerGroupsOption) (result []ConsumerGroupListing, err error, brokerErrs []error) {
	// Convert Go AdminOptions (if any) to C AdminOptions.
	genericOptions := make([]AdminOption, len(options))
	for i := range options {
		genericOptions[i] = options[i]
	}
	cOptions, err := adminOptionsSetup(a.handle, C.RD_KAFKA_ADMIN_OP_LISTSCONSUMERGROUPS, genericOptions)
	if err != nil {
		return nil, err, nil
	}
	defer C.rd_kafka_AdminOptions_destroy(cOptions)
	...
	// Wait for result, error or context timeout.
	rkev, err := a.waitResult(ctx, cQueue, C.RD_KAFKA_EVENT_LISTCONSUMERGROUPS_RESULT)
	if err != nil {
		return nil, err, nil
	}
	defer C.rd_kafka_event_destroy(rkev)
	...
	result = a.cToConsumerGroupListings(cGroups, cGroupCount)
	...
	brokerErrs = a.cToErrorList(cErrs, cErrsCount)
	return result, nil, brokerErrs
}
...
func (a *AdminClient) DescribeConsumerGroups(
	...
	cOptions, err := adminOptionsSetup(a.handle, C.RD_KAFKA_ADMIN_OP_DESCRIBECONSUMERGROUPS, genericOptions)
	if err != nil {
		return nil, err, nil
	}
	defer C.rd_kafka_AdminOptions_destroy(cOptions)
	...
	// Wait for result, error or context timeout.
	rkev, err := a.waitResult(ctx, cQueue, C.RD_KAFKA_EVENT_DESCRIBECONSUMERGROUPS_RESULT)
	if err != nil {
		return nil, err
	}
	defer C.rd_kafka_event_destroy(rkev)
